...
describes what users get. Pick a format with `-o`, or let pragmaticengineer choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pragmaticengineer <command> -o table   # aligned columns for reading
pragmaticengineer <command> -o jsonl   # one JSON object per line, for piping
pragmaticengineer <command> -o json    # a single JSON array
pragmaticengineer <command> -o csv     # spreadsheet friendly
pragmaticengineer <command> -o tsv     # tab-separated
pragmaticengineer <command> -o url     # just the URL column
pragmaticengineer <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pragmaticengineer <command> --fields id,title,url
```
...
```bash
pragmaticengineer <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pragmaticengineer <command>            # a table, because this is a terminal
pragmaticengineer <command> | wc -l    # JSONL, because this is a pipe
```
