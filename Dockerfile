...
RUN apk add --no-cache ca-certificates tzdata \
 && adduser -D -H -u 10001 pragmaticengineer

COPY $TARGETPLATFORM/pragmaticengineer /usr/bin/pragmaticengineer

USER pragmaticengineer

ENTRYPOINT ["/usr/bin/pragmaticengineer"]
